...
@section('title', 'Add Country Levy - Admin')
...
<div class="container-fluid py-4">
    <div class="mb-4">
        ...
        <h2 class="mb-1"><i class="fas fa-plus me-2"></i>Add Country Levy</h2>
        <p class="text-muted mb-0">Configure a new standard levy (wharfage, environmental, etc.)</p>
    </div>
    ...
    <form action="{{ route('admin.country-levies.store') }}" method="POST">
        @csrf
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="levy_code" class="form-control @error('levy_code') is-invalid @enderror" 
                                       value="{{ old('levy_code') }}" required maxlength="20" placeholder="e.g., WHA, ENV, SVC">
                                <small class="text-muted">Short code (max 20 chars)</small>
                                @error('levy_code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="levy_name" class="form-control @error('levy_name') is-invalid @enderror" 
                                   value="{{ old('levy_name') }}" required placeholder="e.g., Wharfage, Environmental Levy">
                            @error('levy_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.0001" name="rate" class="form-control @error('rate') is-invalid @enderror" 
                                       value="{{ old('rate', '0') }}" required min="0">
                                @error('rate')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="rate_type" class="form-select @error('rate_type') is-invalid @enderror" required>
                                    <option value="percentage" {{ old('rate_type', 'percentage') == 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                                    <option value="fixed_amount" {{ old('rate_type') == 'fixed_amount' ? 'selected' : '' }}>Fixed Amount ($)</option>
                                    <option value="per_unit" {{ old('rate_type') == 'per_unit' ? 'selected' : '' }}>Per Unit</option>
                                </select>
                                @error('rate_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="calculation_basis" class="form-select @error('calculation_basis') is-invalid @enderror" required>
                                    <option value="fob" {{ old('calculation_basis', 'fob') == 'fob' ? 'selected' : '' }}>FOB Value</option>
                                    <option value="cif" {{ old('calculation_basis') == 'cif' ? 'selected' : '' }}>CIF Value</option>
                                    <option value="duty" {{ old('calculation_basis') == 'duty' ? 'selected' : '' }}>Customs Duty Amount</option>
                                    <option value="quantity" {{ old('calculation_basis') == 'quantity' ? 'selected' : '' }}>Quantity</option>
                                    <option value="weight" {{ old('calculation_basis') == 'weight' ? 'selected' : '' }}>Weight (kg)</option>
                                </select>
                        ...
                        <div class="mb-3" id="unitField" style="display: none;">
                            ...
                            <input type="text" name="unit" class="form-control" value="{{ old('unit') }}" placeholder="e.g., kg, item, container">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            <div class="form-check">
                                <input type="checkbox" name="applies_to_all_tariffs" value="1" class="form-check-input" id="appliesToAll"
                                       {{ old('applies_to_all_tariffs', true) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3" id="chaptersField" style="display: none;">
                            ...
                            <input type="text" name="applicable_tariff_chapters" class="form-control" 
                                   value="{{ old('applicable_tariff_chapters') }}" placeholder="e.g., 01, 02, 03 (comma-separated)">
                            <small class="text-muted">Only apply to specific HS chapters</small>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Exempt Tariff Codes</label>
                            <input type="text" name="exempt_tariff_codes" class="form-control" 
                                   value="{{ old('exempt_tariff_codes') }}" placeholder="e.g., 9801*, 9802.00 (comma-separated)">
                            <small class="text-muted">Codes exempt from this levy (supports wildcards with *)</small>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Exempt Organization Types</label>
                            <input type="text" name="exempt_organization_types" class="form-control" 
                                   value="{{ old('exempt_organization_types') }}" placeholder="e.g., government, non_profit (comma-separated)">
                            <small class="text-muted">Organization types exempt from this levy</small>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="legal_reference" class="form-control" 
                                   value="{{ old('legal_reference') }}" placeholder="e.g., Customs Act Section 12">
                        </div>
                    </div>
                </div>

                <button type="submit" class="btn btn-primary btn-lg w-100 mb-3">
                    <i class="fas fa-save me-2"></i>Create Levy
                </button>
                <a href="{{ route('admin.country-levies.index') }}" class="btn btn-outline-secondary w-100">Cancel</a>
            </div>
        </div>
    </form>
</div>
